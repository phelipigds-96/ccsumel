...
import { Plus, Search, MapPin, Filter, MoreHorizontal, History, CheckCircle2, Clock, Archive, Tag, Trash2, Pencil } from "lucide-react";
...
import { listOportunidades, saveOportunidade, deleteOportunidade } from "@/lib/oportunidades";
...
function BancoDeOportunidades() {
    ...
    const stats = useMemo(() => {
        ...
        oportunidades.forEach(o => {
            ...
            if (o.status === 'Reservada')
                s.reservadas++;
            if (o.status === 'Utilizada')
                s.utilizadas++;
    ...
    return (<div className="space-y-6">
      <PageHeader title="Banco de Oportunidades" description="Gerencie oportunidades comerciais encontradas nas lojas." actions={!readOnly && (<div className="flex gap-2">
      ...
      <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground bg-card p-3 rounded-lg border">
        ...
        {LOJAS.map(loja => (<span key={loja} className="flex items-center gap-1.5 px-2 py-1 rounded bg-secondary/50">
      ...
      <div className="rounded-xl border bg-card p-4">
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <Input placeholder="Buscar por descrição, GTIN ou código..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9"/>
          </div>
        ...
        <div className="overflow-x-auto rounded-lg border">
          <Table>
            ...
            <TableBody>
              {loading ? (<TableRow><TableCell colSpan={9} className="h-24 text-center">Carregando...</TableCell></TableRow>) : filtered.length === 0 ? (<TableRow><TableCell colSpan={9} className="h-24 text-center text-muted-foreground">Nenhuma oportunidade encontrada.</TableCell></TableRow>) : (filtered.map(o => (<TableRow key={o.id} className="group">
                    <TableCell>
                      <Badge variant="outline" className={`flex w-fit items-center gap-1.5 font-medium text-[10px] ${o.status === 'Disponível' ? 'bg-amber-50 text-amber-700 border-amber-200' :
              ...
              {editing?.descricao && (<>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input type="number" value={editing.quantidade_aproximada} onChange={e => setEditing({ ...editing, quantidade_aproximada: Number(e.target.value) })}/>
                    </div>
                    <div className="space-y-2">
                      <Label>Prioridade</Label>
                      <Select value={editing.prioridade} onValueChange={(v) => setEditing({ ...editing, prioridade: v })}>
                        <SelectTrigger><SelectValue /></SelectTrigger>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select value={editing.motivo} onValueChange={(v) => setEditing({ ...editing, motivo: v })}>
                        <SelectTrigger><SelectValue /></SelectTrigger>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input type="date" value={editing.data_coleta} onChange={e => setEditing({ ...editing, data_coleta: e.target.value })}/>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input type="date" value={editing.validade || ""} onChange={e => setEditing({ ...editing, validade: e.target.value })}/>
                    </div>
                    <div className="space-y-2">
                      <Label>Lote (Opcional)</Label>
                      <Input value={editing.lote || ""} onChange={e => setEditing({ ...editing, lote: e.target.value })}/>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <Label>Observações</Label>
                    <Textarea value={editing.observacoes || ""} onChange={e => setEditing({ ...editing, observacoes: e.target.value })} placeholder="Detalhes adicionais sobre a coleta..."/>
                  </div>
                </>)}
            </div>
      ...
      <Dialog open={!!historyOpen} onOpenChange={(v) => !v && setHistoryOpen(null)}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          {historyOpen && (<div className="space-y-6 pt-4">
              ...
              <div className="relative space-y-4 pl-6 before:absolute before:left-2 before:top-2 before:h-[calc(100%-16px)] before:w-px before:bg-border">
                ...
                {historyOpen.campanha_id && (<div className="relative">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      {historyOpen.data_utilizacao && (<span className="text-[10px] text-muted-foreground">Em {new Date(historyOpen.data_utilizacao).toLocaleString()}</span>)}
                    </div>
                  </div>)}

                {historyOpen.status === 'Utilizada' && (<div className="relative">
