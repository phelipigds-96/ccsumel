...
import { Plus, Search, Pencil, Trash2, Printer, Tag, ArrowLeft, Calendar, Package, ChevronRight, Paperclip, Upload, FileText, Image as ImageIcon, X, DollarSign, Columns3, Check, Eye, FileDown, ListOrdered, Share2, Target, LayoutGrid, List, ArrowUpDown } from "lucide-react";
...
import { useCampanhasStore, campanhasStore, categoriaCampanha, statusCampanha, } from "@/lib/campanhas-store";
...
import { brl, fmtDate, printCampanhaPDF, MateriaisViewer, CampanhaQuickView, } from "@/components/campanha-quick-view";
...
import { listOportunidades, updateOportunidadeStatus } from "@/lib/oportunidades";
function CampanhasModulo() {
    ...
    const saveOferta = (o) => campanhasStore.setOfertas((prev) => {
    ...
    const deleteOferta = async (id) => {
        ...
        if (o) {
            ...
            const { data } = await supabase.from('oportunidades').select('id').eq('gtin', o.gtin).eq('campanha_id', o.campanhaId).maybeSingle();
            if (data && data.id) {
    ...
    const saveCampanha = (c) => campanhasStore.setCampanhas((prev) => {
    ...
    return selected ? (<CampanhaDetalhe campanha={selected} ofertas={ofertas.filter((o) => o.campanhaId === selected.id)} onBack={() => setSelectedId(null)} onSaveOferta={saveOferta} onDeleteOferta={deleteOferta}/>) : (<CampanhasList campanhas={visiveis} ofertas={ofertas} onOpen={setSelectedId} onSave={saveCampanha} onDelete={deleteCampanha}/>);
}
function CampanhasList({ campanhas, ofertas, onOpen, onSave, onDelete }) {
    const { user } = useAuth();
    ...
    const [sortField, setSortField] = useState("dataInicial");
    const [sortOrder, setSortOrder] = useState("desc");
    const [dialogOpen, setDialogOpen] = useState(false);
    const [editing, setEditing] = useState(null);
    const [deleteId, setDeleteId] = useState(null);
    const [quickView, setQuickView] = useState(null);
    const [cvCampanha, setCvCampanha] = useState(null);
    const [dpCampanha, setDpCampanha] = useState(null);
    ...
    const filtered = useMemo(() => {
        ...
        return sortedVisiveis
            .filter(c => {
            if (q && !c.nome.toLowerCase().includes(q) && !c.descricao.toLowerCase().includes(q))
                return false;
            if (fStatus !== "todos" && statusCampanha(c) !== fStatus)
                return false;
            return true;
        });
    ...
    const renderCard = (c, opts = {}) => {
        ...
        return (<div key={c.id} className={"group relative flex flex-col overflow-hidden rounded-2xl border bg-card transition " +
                (opts.destaque
                    ? "border-navy/30 shadow-[0_1px_2px_rgba(11,31,58,0.06)] hover:border-navy/60 hover:shadow-lg"
                    : "hover:border-primary/30 hover:shadow-lg")}>
        <span className={"absolute inset-x-0 top-0 h-0.5 " + (opts.destaque ? "bg-navy" : "bg-primary/70")}/>
        <div className="flex flex-1 flex-col p-5">
          ...
          <div className="mb-4 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-muted-foreground">
            ...
            {c.materiais.length > 0 && (<span className="flex items-center gap-1.5" title="Materiais de apoio">
          ...
          <div className="mt-auto flex flex-wrap items-center justify-between gap-2 border-t pt-3">
            <div className="flex items-center gap-0.5">
              ...
              <Button size="icon" variant="ghost" className="h-8 w-8 text-muted-foreground hover:text-foreground" title="Imprimir listagem de produtos" onClick={() => { printCampanhaPDF(c, ofertas.filter((o) => o.campanhaId === c.id)); toast.success("PDF aberto em nova aba."); }}>
    ...
    return (<div>
      <PageHeader title="Campanhas" description="Gerencie as campanhas e ofertas da Central Sumel." actions={!readOnly ? (<Button onClick={openNew} className="bg-primary hover:bg-primary/90">
      ...
      <div className="rounded-xl border bg-card p-4 mb-4 flex flex-col md:flex-row items-center gap-3">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <div className="flex items-center rounded-lg border bg-background p-1 shadow-sm">
            <Button variant={viewMode === "grid" ? "default" : "ghost"} size="sm" className="h-8 px-3 rounded-md text-xs" onClick={() => setViewMode("grid")}>
              <LayoutGrid className="mr-2 h-3.5 w-3.5"/>
              Blocos
            </Button>
            <Button variant={viewMode === "list" ? "default" : "ghost"} size="sm" className="h-8 px-3 rounded-md text-xs" onClick={() => setViewMode("list")}>
      ...
      {filtered.length === 0 ? (<div className="rounded-xl border border-dashed p-10 text-center text-muted-foreground">
          Nenhuma campanha ativa ou programada.
        </div>) : viewMode === "grid" ? (<div className="space-y-6">
          {futuras.length > 0 && (<section>
        ...
        </div>) : (<div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm">
            ...
            <Table>
              <TableHeader>
                ...
                if (sortField === 'nome')
                  ...
                  <TableHead className="cursor-pointer" onClick={() => {
                if (sortField === 'dataInicial')
                    setSortOrder(sortOrder === 'asc' ? 'desc' : 'asc');
                else {
                    setSortField('dataInicial');
                    setSortOrder('asc');
                }
            }}>
                  ...
                  <TableHead className="cursor-pointer" onClick={() => {
                if (sortField === 'dataFinal')
                    setSortOrder(sortOrder === 'asc' ? 'desc' : 'asc');
                else {
                    setSortField('dataFinal');
                    setSortOrder('asc');
                }
            }}>
              ...
              <TableBody>
                ...
                return (<TableRow key={c.id} className="group border-border/60 transition-colors hover:bg-accent/60">
                      <TableCell>
      ...
      <CampanhaDialog open={dialogOpen} onOpenChange={(v) => { setDialogOpen(v); if (!v)
        setEditing(null); }} campanha={editing} setCampanha={setEditing} onSave={save}/>

      <CampanhaQuickView campanha={quickView} ofertas={quickView ? ofertas.filter((o) => o.campanhaId === quickView.id) : []} onOpenChange={(v) => { if (!v)
        setQuickView(null); }} onAbrir={(id) => { setQuickView(null); onOpen(id); }}/>

      {cvCampanha && (<CresceVendasDialog campanha={cvCampanha} ofertas={ofertas.filter((o) => o.campanhaId === cvCampanha.id)} open={!!cvCampanha} onOpenChange={(v) => { if (!v)
            setCvCampanha(null); }}/>)}

      {dpCampanha && (<DescricaoPrecoDialog campanha={dpCampanha} ofertas={ofertas.filter((o) => o.campanhaId === dpCampanha.id)} open={!!dpCampanha} onOpenChange={(v) => { if (!v)
            setDpCampanha(null); }}/>)}
...
function CampanhaDetalhe({ campanha, ofertas, onBack, onSaveOferta, onDeleteOferta }) {
    const { user } = useAuth();
    const readOnly = !user?.isAdmin || !!user?.readOnly;
    const [search, setSearch] = useState("");
    ...
    const { isVisible, toggle } = useColumnPrefs("campanhas.ofertas", COLUMNS.map((c) => c.key), { filiais: false });
    const visibleCount = COLUMNS.filter((c) => isVisible(c.key)).length + 1; // +Ações
    const filtered = useMemo(() => {
        const q = search.trim().toLowerCase();
        return ofertas.filter(o => {
            if (q && ![o.codigo, o.descricao, o.fornecedor].some(f => f.toLowerCase().includes(q)))
                return false;
            if (fCategoria !== "todas" && o.categoria !== fCategoria)
                return false;
            if (fStatus !== "todos" && o.status !== fStatus)
                return false;
            if (fClube === "sim" && !o.clubeSumel)
                return false;
            if (fClube === "nao" && o.clubeSumel)
                return false;
            if (fSellout === "com" && !o.selloutTemVerba)
                return false;
            if (fSellout === "sem" && o.selloutTemVerba)
                return false;
            return true;
        });
    ...
    const openEdit = (o) => { setEditing({ ...o }); setIsNew(false); setDialogOpen(true); };
    const save = async () => {
        if (!editing)
            return;
        ...
        if (isNew) {
            ...
            const { data: op } = await supabase.from('oportunidades').select('id').eq('gtin', editing.gtin).eq('status', 'Disponível').maybeSingle();
            if (op && op.id) {
                await updateOportunidadeStatus(op.id, isPublicada ? 'Utilizada' : 'Reservada', campanha.id);
            }
        }
    ...
    return (<div>
      ...
      <PageHeader title={campanha.nome} description={`Vigência ${fmtDate(campanha.dataInicial)} → ${fmtDate(campanha.dataFinal)}  •  ${campanha.descricao || "Ofertas desta campanha."}`} actions={<>
      ...
      <BancoOportunidadesSelectDialog open={oportunidadesOpen} onOpenChange={setOportunidadesOpen} onSelect={(o) => {
            const novaOferta = {
      ...
      <div className="mb-4 rounded-2xl border border-border/70 bg-card/80 p-3 shadow-sm backdrop-blur flex flex-wrap gap-2">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            ...
            return (<DropdownMenuItem key={col.key} onSelect={(e) => { e.preventDefault(); toggle(col.key); }} className="flex items-center justify-between cursor-pointer">
                  <span>{col.label}</span>
      ...
      <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm">
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {filtered.length === 0 ? (<TableRow className="hover:bg-transparent">
                ...
                </TableRow>) : filtered.map((o) => {
            const desconto = o.precoNormal > 0 && o.precoPromocional > 0
                ? Math.round((1 - o.precoPromocional / o.precoNormal) * 100)
                : 0;
            return (<TableRow key={o.id} className="group border-border/60 transition-colors hover:bg-accent/60">
                  ...
                  {isVisible("precoPromocional") && (<TableCell className="text-right">
                      <div className="inline-flex items-center gap-1.5">
                        {desconto > 0 && (<span className="rounded-full bg-primary/10 px-1.5 py-0.5 text-[10px] font-bold text-primary">-{desconto}%</span>)}
                        <span className="font-semibold tabular-nums text-primary">{brl(o.precoPromocional)}</span>
                      </div>
                    </TableCell>)}
                  {isVisible("clubeSumel") && (<TableCell className="text-center">
                  ...
                  {isVisible("sellout") && (<TableCell className="text-right">
                      {o.selloutTemVerba ? (<div className="flex flex-col items-end leading-tight">
                  ...
                  <TableCell className="text-right whitespace-nowrap">
                    {readOnly ? (<span className="text-xs text-muted-foreground">—</span>) : (<div className="inline-flex items-center gap-1 opacity-60 transition-opacity group-hover:opacity-100">
      ...
      <OfertaDialog open={dialogOpen} onOpenChange={(v) => { setDialogOpen(v); if (!v)
        setEditing(null); }} oferta={editing} setOferta={setEditing} onSave={save} filiaisPermitidas={campanha.filiais ?? []}/>
...
function CampanhaDialog({ open, onOpenChange, campanha, setCampanha, onSave }) {
    if (!campanha)
        return null;
    const upd = (k, v) => setCampanha({ ...campanha, [k]: v });
      ...
      <DialogContent className="max-w-2xl max-h-[95vh] overflow-y-auto p-0">
        ...
        <div className="grid gap-4 py-2">
          ...
          <Field label="Status">
            <div className="flex flex-wrap items-center gap-3 rounded-md border p-3">
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="checkbox" className="h-4 w-4 accent-primary" checked={campanha.status === "Rascunho"} onChange={(e) => upd("status", (e.target.checked ? "Rascunho" : statusCampanha({ ...campanha, status: "Ativa" })))}/>
          ...
          <Field label="Filiais participantes (selecione uma ou mais)">
            ...
            return (<label key={f} className="flex items-center gap-2 text-sm cursor-pointer">
          ...
          <MateriaisUploader campanhaId={campanha.id} materiais={campanha.materiais} onChange={(m) => upd("materiais", m)}/>
        </div>
        <DialogFooter>
...
function isImage(tipo) { return tipo.startsWith("image/"); }
function MateriaisUploader({ campanhaId, materiais, onChange, }) {
    ...
    return (<div className="grid gap-2">
      ...
      {materiais.length > 0 && (<ul className="grid gap-1.5">
          {materiais.map((m) => (<li key={m.path} className="flex items-center gap-2 rounded-md border bg-card px-3 py-2 text-sm">
...
function OfertaDialog({ open, onOpenChange, oferta, setOferta, onSave, filiaisPermitidas }) {
    ...
    if (!oferta)
        return null;
    const upd = (k, v) => setOferta({ ...oferta, [k]: v });
    ...
    const priceAt = (pct) => +(oferta.precoNormal * (1 - pct / 100)).toFixed(2);
    const setPromoByPct = (pct) => upd("precoPromocional", Math.max(priceAt(pct), 0));
    const descontoTone = descontoPct <= 0 ? "text-muted-foreground"
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[95vh] overflow-y-auto p-0">
        ...
        {historico.length > 0 && (<div className="rounded-lg border border-amber-300 bg-amber-50 p-3 mb-1">
            <div className="text-sm font-semibold text-amber-900">
              Este produto já participou de {historico.length} campanha{historico.length > 1 ? "s" : ""}
            </div>
            <div className="mt-2 space-y-1.5 max-h-40 overflow-y-auto">
              {historico.map(({ o, c }) => (<div key={o.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md bg-card px-2.5 py-1.5 text-xs">
        ...
        <div className="grid gap-4 py-2 grid-cols-1 md:grid-cols-2">
          ...
          <div className="md:col-span-2 rounded-lg border p-4 bg-card">
            ...
            return (<button key={pct} type="button" onClick={() => setPromoByPct(pct)} disabled={oferta.precoNormal <= 0} className={`rounded-lg border p-3 text-left transition hover:border-primary hover:shadow-sm disabled:opacity-50 disabled:cursor-not-allowed ${ativo ? "border-primary bg-primary/5" : "bg-card"}`}>
            ...
            <Field label="Preço Promocional (R$) — ou informe manualmente">
              <Input ref={promoRef} type="number" step="0.01" value={oferta.precoPromocional} onChange={(e) => upd("precoPromocional", parseFloat(e.target.value) || 0)}/>
            </Field>

            <div className="mt-4">
              <Field label="Códigos de barras cresce vendas">
                <Textarea value={oferta.gtinsCresceVendas} onChange={(e) => upd("gtinsCresceVendas", e.target.value)} placeholder="Informe um código de barras por linha (ou separados por vírgula) para variações do produto" rows={3}/>
              </Field>
            </div>
          </div>
          ...
          <Field label="Filiais (herdadas da campanha — desmarque para excluir alguma)" className="md:col-span-2">
            {filiaisPermitidas.length === 0 ? (<div className="rounded-md border border-dashed p-3 text-xs text-muted-foreground">
                Nenhuma filial foi selecionada na campanha. Edite a campanha e escolha as filiais participantes.
              </div>) : (<div className="grid grid-cols-2 sm:grid-cols-4 gap-2 rounded-md border p-3">
          ...
          <Field label="Status (definido pela campanha)">
            <Select value={oferta.status} disabled onValueChange={(v) => upd("status", v)}>
          ...
          <div className="md:col-span-2 rounded-lg border p-4 bg-navy/5">
            ...
            {oferta.selloutTemVerba && (<div className="grid gap-3 md:grid-cols-2">
                <Field label="Fornecedor da verba">
                  <Input value={oferta.selloutFornecedor} onChange={(e) => upd("selloutFornecedor", e.target.value)} placeholder="Ex: Ambev"/>
                </Field>
                <Field label="Valor da verba (R$)">
                  <Input type="number" step="0.01" value={oferta.selloutValor} onChange={(e) => upd("selloutValor", parseFloat(e.target.value) || 0)}/>
                </Field>
                <Field label="Observações / condições de cobrança" className="md:col-span-2">
                  <Textarea rows={2} value={oferta.selloutObs} onChange={(e) => upd("selloutObs", e.target.value)} placeholder="Ex: NF emitida ao final da campanha, referência do contrato, contato do comprador..."/>
                </Field>
              </div>)}
            {!oferta.selloutTemVerba && (<p className="text-xs text-muted-foreground">Ative quando o fornecedor pagar uma verba específica por esta oferta (para lembrar de cobrar depois).</p>)}
          </div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)}>Cancelar</Button>
...
function Field({ label, children, className = "" }) {
    return (<div className={`grid gap-1.5 w-full ${className}`}>
      <Label className="text-xs text-muted-foreground">{label}</Label>
      {children}
    </div>);
}
function BancoOportunidadesSelectDialog({ open, onOpenChange, onSelect }) {
    ...
    const filtered = useMemo(() => {
        ...
        return opts.filter(o => o.descricao.toLowerCase().includes(q) ||
            o.gtin?.includes(q) ||
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col p-0">
        <DialogHeader className="p-6 pb-0">
          ...
          <div className="relative mt-4">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground"/>
            <Input placeholder="Buscar por descrição, código ou barras..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 rounded-full border-border/70"/>
          </div>
        </DialogHeader>

        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (<div className="flex flex-col items-center justify-center py-12 gap-2 text-muted-foreground">
              <div className="h-8 w-8 animate-spin rounded-full border-2 border-primary border-t-transparent"/>
              <p className="text-sm">Carregando oportunidades...</p>
            </div>) : filtered.length === 0 ? (<div className="flex flex-col items-center justify-center py-12 text-muted-foreground text-center">
              ...
              {filtered.map(o => (<button key={o.id} onClick={() => onSelect(o)} className="flex items-center justify-between rounded-xl border border-border/70 p-4 text-left transition-all hover:border-primary hover:bg-primary/5 hover:shadow-sm">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-navy">{o.descricao}</span>
                      <Badge variant="outline" className={`text-[10px] h-4 ${o.prioridade === 'Alta' ? 'bg-red-50 text-red-600 border-red-200' :
